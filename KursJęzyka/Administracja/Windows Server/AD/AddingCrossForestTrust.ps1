...
$ADINSTALLHT = @{
  DomainName                    = 'las2.Com' # Forest Root
...
# 7. Adjust DNS on KAPDC1 to resolve domena.com from DC1
$CFHT = @{
   Name          = 'domena.com'
...
$SB = {
  ...
  Add-DnsServerConditionalForwarderZone @CFHT
  # Test it
  Resolve-DNSName -Name KAPDC1.las2.Com | Format-Table
}  

# 10. Create Credentials to Run A Command on DC1
$URK   = 'Domena\Administrator'
$PRK   = ConvertTo-SecureString 'Pa$$w0rd' -AsPlainText -Force
$CREDRK =  [PSCredential]::New($URK,$PRK)

# 11. Set WinRM
$PATH = 'WSMan:\localhost\Client\TrustedHosts'
Set-Item -Path $PATH -Value '*.domena.com' -Force

# 12. Run the Script Block On DC1
$NZHT = @{
  Computername = 'DC1.domena.com'
  Script       = $SB
  Credential  = $CREDRK
}
Invoke-Command @NZHT 

# 13. Get domena.com and las2.Com details
$Reskit       = 'domena.com'
$User         = 'Administrator'
$UserPW       = 'Pa$$w0rd'
